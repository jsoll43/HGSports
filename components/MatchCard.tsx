...
export function MatchCard({ match, player }: { match: Match; player?: Player }) {
  const opponent = player ? opponentTeam(player, match) : null
    ...
    <article className="grid gap-4 rounded-lg border border-cyan-100 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-navy">{player ? `vs ${opponent?.name}` : matchTitle(match)}</h3>
